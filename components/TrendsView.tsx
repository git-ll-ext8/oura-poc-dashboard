...
import { DAYS, members, weeklySeries } from "@/lib/data";

const CHARTS: { key: "sleep" | "hrv" | "steps"; title: string }[] = [
  { key: "sleep", title: "Sleep Score — Past 7 Days" },
  { key: "hrv", title: "HRV (ms) — Past 7 Days" },
  { key: "steps", title: "Daily Steps — Past 7 Days" },
];

function buildData(key: "sleep" | "hrv" | "steps") {
  return DAYS.map((day, i) => {
    const row: Record<string, string | number> = { day };
    for (const m of members) {
      row[m.name] = weeklySeries(m, key)[i];
    }
    return row;
  });
}
    ...
    <div className="tab-page active">
      <div className="page-header">
        <div>
          ...
          <div className="page-date">Last 7 days · All members</div>
        </div>
      </div>

      {CHARTS.map((chart, idx) => {
        const data = buildData(chart.key);
        return (
